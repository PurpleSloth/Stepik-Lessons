...
информации, необходимо вывести напротив него прочерк, например:
...
result_table = [[i + 1, 0] for i in range(11)]
...
for clas in classes:
    for k in range(len(classes[clas])):
        sum_in_class += int(classes[clas][k][1])
    if len(classes[clas]) == 0:
        result_table[clas - 1][1] = '-'
    else:    
        result_table[clas - 1][1] = sum_in_class / len(classes[clas])
    sum_in_class = 0    
